...
int main(int argc, char** argv) {
    ...
    int N = 100;    //number of initial grids in each dimension
    double Tend = 2.5;    //running time
    ...
    for (int n = 0; n < nt; n++)
    {
        ...
        if (n == 0)
        {
            ...
            for (int iid = 0; iid < P; iid++)
            {
                if (ID == iid)
                {
                    outfile0.open("output0.dat", ios::app);
                    for (int i = 0; i < Nc; i++)
                    {
                        itempc0 = i + istart - igstart; 
                        if (I[itempc0])
                        {
                            for (int j = 0; j < 4; j++)
                            {
                                itempf0 = itempc0*4 + j;
                                outfile0 << I[itempc0] << " " << (istart+i)*4+j << " " << 
                                        Uf[0][itempf0] << " " << Uf[1][itempf0] << " " << Uf[2][itempf0] << " " << ID << endl;
                            }
                        }
                        else
                        {
                            outfile0 << I[itempc0] << " " << istart+i << " " << 
                                    Uc[0][itempc0] << " " << Uc[1][itempc0] << " " << Uc[2][itempc0] << " " << ID <<endl;
                        }
                    }
                    outfile0.close();
                }
        ...
        // Output the solution
        ts += dt;
        double tout = Tend/double(nout)*double(iout+1);
        if (ts >= tout || n == nt-1)
        {
            ofstream outfile;
            char outs[20];
            sprintf(outs, "output%d.dat", iout+1);
            for (int iid = 0; iid < P; iid++)
            {
                if (ID == iid)
                {
                    outfile.open(outs, ios::app);
                    /*
                    if (ID == 0)
                    {
                        ...
                        if (I[itempc])
                        {
                            for (int j = 0; j < 4; j++)
                            {
                                ...
                                outfile << I[itempc] << " " << (istart+i)*4+j << " " << 
                                        Uf[0][itempf] << " " << Uf[1][itempf] << " " << Uf[2][itempf] << " " << ID <<endl;
                            }
                        }
                        else
                        {
                            outfile << I[itempc] << " " << istart+i << " " << 
                                    Uc[0][itempc] << " " << Uc[1][itempc] << " " << Uc[2][itempc] << " " << ID <<endl;
                        }
                    }
                    outfile.close();
                }
    ...
    double maxh = *max_element(Uf[0].begin(), Uf[0].end());
    double globalmax; 
    MPI_Reduce(&maxh, &globalmax, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
    if (ID == 0)
    {
        cout << "Maximum water height = " << globalmax << endl;
    }
